                ...
                <header className="flex items-start justify-between gap-4 border-b border-[var(--organisation-border)] p-6">
                    <div className="flex min-w-0 items-center gap-4">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 text-sm text-[color:var(--organisation-text)]/60">{formatLabel(official.role)}</p>
                        </div>
                    </div>
                    <button type="button" className="rounded-xl border border-[var(--organisation-border)] bg-[var(--organisation-background)] p-2.5 text-[color:var(--organisation-text)]/75 hover:border-[var(--organisation-accent)]" onClick={onClose} aria-label="Close official profile"><X className="h-5 w-5" /></button>
                </header>
                <div className="flex-1 space-y-6 overflow-y-auto p-6">
                    <div className="grid gap-3 sm:grid-cols-2">
                        <div className="rounded-2xl border border-[var(--organisation-border)] bg-[var(--organisation-background)] p-4"><p className="text-xs font-bold uppercase tracking-wider text-[color:var(--organisation-text)]/50">Status</p><p className="mt-2 font-semibold">{formatLabel(official.status)}</p></div>
                        <div className="rounded-2xl border border-[var(--organisation-border)] bg-[var(--organisation-background)] p-4"><p className="text-xs font-bold uppercase tracking-wider text-[color:var(--organisation-text)]/50">Verification</p><p className="mt-2 flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4 text-[var(--organisation-accent)]" />{formatLabel(official.verification_status)}</p></div>
                    </div>
                    <section className="space-y-3 rounded-2xl border border-[var(--organisation-border)] bg-[var(--organisation-background)] p-5">
                        <h3 className="font-bold">Contact</h3>
                        <div className="flex items-center gap-3 text-sm text-[color:var(--organisation-text)]/75"><Mail className="h-4 w-4 text-[var(--organisation-accent)]" /><span className="break-all">{official.email || 'Not provided'}</span></div>
                        <div className="flex items-center gap-3 text-sm text-[color:var(--organisation-text)]/75"><Phone className="h-4 w-4 text-[var(--organisation-accent)]" /><span>{official.phone || 'Not provided'}</span></div>
                        <div className="flex items-center gap-3 text-sm text-[color:var(--organisation-text)]/75"><MapPin className="h-4 w-4 text-[var(--organisation-accent)]" /><span>{location || 'Not provided'}</span></div>
                    </section>
                    {official.biography && <section className="rounded-2xl border border-[var(--organisation-border)] bg-[var(--organisation-background)] p-5"><h3 className="font-bold">Biography</h3><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[color:var(--organisation-text)]/75">{official.biography}</p></section>}
                </div>
            </aside>
        </div>
    )
