...
function createImagePath(fileName: string) {
    ...
    return `articles/${Date.now()}-${uniqueId}-${createSafeFileName(fileName)}`;
}
...
export function ArticlesManager({ onArticlesChanged }: ArticlesManagerProps) {
    ...
    function updateForm<Key extends keyof ArticleFormState>(
        key: Key,
    ...
    async function uploadArticleImage(file: File) {
        ...
        if (file.size > MAX_IMAGE_SIZE_BYTES) {
            setErrorMessage("The image must be no larger than 5 MB.");
            return;
        }

        setUploadingImage(true);

        const imagePath = createImagePath(file.name);
        ...
        if (uploadError) {
            ...
            setErrorMessage(uploadError.message || "Unable to upload the image.");
        ...
        setForm((current) => ({
            ...current,
            imageUrl: publicUrl,
            imageAlt: current.imageAlt.trim() || current.title.trim(),
        }));
    ...
    async function saveArticle() {
        ...
        const payload = {
            ...
            hero: form.hero.trim(),
            read_time: form.readTime.trim() || "3 min read",
    ...
    async function updateStatus(article: DbArticle, status: ArticleStatus) {
        setMessage(null);
        setErrorMessage(null);
        ...
        if (error) {
            ...
            setErrorMessage("Unable to update article status.");
            return;
        }

        setMessage(`Article status changed to ${status}.`);
    ...
    return (
        <div>
            <div className="adminWorkspaceHeader">
                <div>
                    <h3>Manage Black History Articles</h3>

                    <p className="muted">
                        Create, edit, review and publish content for the Black History Hub.
                    </p>
                </div>
                    ...
                    <div className="adminFormGrid">
                        ...
                        <label>
                            ...
                            <select
                                value={form.category}
                                onChange={(event) =>
                                    updateForm("category", event.target.value as ArticleCategory)
                                }
                            >
                        ...
                        <label>
                            ...
                            <select
                                value={form.status}
                                onChange={(event) =>
                                    updateForm("status", event.target.value as ArticleStatus)
                                }
                            >
                        ...
                        <div className="adminFormFullWidth articleImageUploadSection">
                            <div>
                                <span className="articleImageUploadLabel">Hero image</span>

                                <p className="muted articleImageUploadHelp">
                                    Upload a JPEG, PNG or WebP image. Maximum file size: 5 MB.
                                </p>
                            </div>
                  ...
                  {uploadingImage ? "Uploading..." : "Choose image"}
                            ...
                            {form.imageUrl && (
                                <div className="articleImagePreview">
                                    <img
                                        src={form.imageUrl}
                                        alt={form.imageAlt || form.title || "Article hero preview"}
                                    />
                        ...
                        <label className="adminFormFullWidth">
                            <span>Further-reading links</span>
                ...
                {loading ? (
                    ...
                    {article.status}
                            ...
                            <div className="articleAdminRecordActions">
                                <button type="button" onClick={() => startEditing(article)}>
                                    Edit
                                </button>
                                ...
                                {article.status === "published" && (
                                    <button
                                        type="button"
                                        onClick={() => void updateStatus(article, "draft")}
                                    >
                                        Unpublish
                                    </button>
                                )}
                    ...
                    <p className="muted">No articles have been created.</p>
                )}
            </div>
        </div>
    );
